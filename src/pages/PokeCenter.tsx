...
type WeatherCondition = 'CLEAR' | 'CLOUDY' | 'FOG' | 'RAIN' | 'SNOW' | 'STORM'

function weatherFromCode(code: number): WeatherCondition {
...
// 12×8 pixel art weather sprites: palette[0]=transparent, rest=colors
const WEATHER_ART: Record<WeatherCondition, { palette: string[]; rows: string[] }> = {
  CLEAR: {
    palette: ['', '#5ab4f0', '#f0d800', '#fff040'],
    rows: ['111111111111','111111111111','111122111111','112333211111',
           '112333211111','111222111111','111111111111','111111111111'],
  },
  CLOUDY: {
    palette: ['', '#9ab0b8', '#dde8ec', '#f4f8fa'],
    rows: ['111111111111','112222221111','122332221111','222333322211',
           '122222222211','112222221111','111111111111','111111111111'],
  },
  FOG: {
    palette: ['', '#b8c8cc', '#ddeaee'],
    rows: ['122222222221','111111111111','122222222221','111111111111',
           '122222222221','111111111111','122222222221','111111111111'],
  },
  RAIN: {
    palette: ['', '#788090', '#b0bcc8', '#d0dce8', '#4898e0'],
    rows: ['111111111111','114444441111','144333441111','444333444411',
           '111111111111','141111411111','114111141111','111411114111'],
  },
  SNOW: {
    palette: ['', '#8090a0', '#c8d8e4', '#f0f4f8', '#ffffff'],
    rows: ['111111111111','114444441111','144334441111','444334444411',
           '111111111111','141111411111','111411114111','141111141111'],
  },
  STORM: {
    palette: ['', '#383848', '#505868', '#787888', '#f0d800'],
    rows: ['111111111111','122222221111','122222221111','222222222221',
           '111141111111','111144111111','111141111111','111111111111'],
  },
}

function WeatherPixelArt({ condition }: { condition: WeatherCondition }) {
  const art = WEATHER_ART[condition]
  const cols = 12, rows = 8, px = 2
  const rects: React.ReactElement[] = []
  art.rows.forEach((row, r) => {
    for (let c = 0; c < cols; c++) {
      const idx = parseInt(row[c] ?? '0', 10)
      if (idx === 0) continue
      const color = art.palette[idx]
      if (!color) continue
      rects.push(<rect key={`${r}-${c}`} x={c * px} y={r * px} width={px} height={px} fill={color} />)
    }
  })
  return (
    <svg viewBox={`0 0 ${cols * px} ${rows * px}`} width={cols * px} height={rows * px}
...
function WeatherWindow() {
      ...
      async ({ coords: { latitude: lat, longitude: lon } }) => {
        try {
          const res = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat.toFixed(4)}&longitude=${lon.toFixed(4)}&current=weather_code,temperature_2m&temperature_unit=celsius`,
          )
          const data = await res.json()
          setTemp(Math.round(data.current.temperature_2m as number))
          setCondition(weatherFromCode(data.current.weather_code as number))
          setStatus('ok')
  ...
  if (status === 'denied' || !condition) {
    return (
      <div style={boxStyle}>
        <span style={{ fontFamily: FONT, fontSize: 6, color: GBC_MUTED }}>NO GPS</span>
      </div>
    )
  }
  return (
    <div style={boxStyle}>
      <WeatherPixelArt condition={condition} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <span style={{ fontFamily: FONT, fontSize: 6, color: GBC_TEXT }}>{condition}</span>
        <span style={{ fontFamily: FONT, fontSize: 6, color: GBC_AMBER }}>{temp}°C</span>
      </div>
    </div>
  )
}
